...
for min_clus  in range(1,40):

    for mins in range(1,min_clus):
        ...
        ax1.set_xlim([-0.1, 1])
        cluster_labels = hdbscan.HDBSCAN(min_cluster_size=20, min_samples=5).fit_predict(file_RADEC_array)
        ...
        ax2.scatter(file_RADEC_array[:, 0],file_RADEC_array[:, 1], marker='.', s=30, lw=0, alpha=0.7,
              c=colors)
