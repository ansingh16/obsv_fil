...
def parallel_it(f):
                ...
                for j in range(len(nearby_gal)):
                    ...
                    #filament_out = open('Filament_output.csv','a')

                return all_filament_gal,all_filament_gal_dist,all_filament_gal_dist_fil,all_filament_gal_dist_tot_fil
...
if __name__ == '__main__':
    ...
    for sim_name, sim_size in mySims:
        ...
        np.savetxt("Data_Groups_and_cluster.csv", Data, header='x,y,z,SM,Metal,u_minus_r', delimiter=',')
    ...
    data_no_cluster = df1.loc[
        ~df1.set_index(list(df1.columns)).index.isin(df2.set_index(list(df2.columns)).index)].values
    ...
    filament_out.write('#x,y,z,d_per(Mpc),d_long(Mpc),length_fil(Mpc),SM,Metal,u_minus_r\n')
    ...
    FIL = np.where(lengths>5.0)[0]+1



    pool = Pool(processes=50)
    result = pool.map(parallel_it,FIL)

    all_filament_gal1 = []
    all_filament_gal_dist1 = []
    all_filament_gal_dist_fil1 = []
    all_filament_gal_dist_tot_fil1 = []

    for r in range(len(result)):
        allfilamentgal, allfilamentgaldist, allfilamentgaldistfil, allfilamentgaldisttotfil = result[r]

        all_filament_gal1 += allfilamentgal
        all_filament_gal_dist1 += allfilamentgaldist
        all_filament_gal_dist_fil1 += allfilamentgaldistfil
        all_filament_gal_dist_tot_fil1 += allfilamentgaldisttotfil

    S = np.array(all_filament_gal1)

    print S.shape,np.unique(S).shape

    np.savetxt(filament_out, np.array([
                                       data_no_cluster[all_filament_gal1, 0],
                                       data_no_cluster[all_filament_gal1, 1],
                                       data_no_cluster[all_filament_gal1, 2],
                                       all_filament_gal_dist1,
                                       all_filament_gal_dist_fil1,
                                       all_filament_gal_dist_tot_fil1,
                                       data_no_cluster[all_filament_gal1, 3],
                                       data_no_cluster[all_filament_gal1, 4],
                                       data_no_cluster[all_filament_gal1, 5]]).T,
               delimiter=',', fmt='%f,%f,%f,%f,%f,%f,%f,%f,%f')
    ...
    with open('Field_galaxies.csv','w') as outfile:
        ...
        outfile.write('#x,y,z,SM,Metal,u_minus_r\n')
