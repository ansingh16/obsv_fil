...
data_all_mass = pd.read_csv('Data_all_mass.csv',header=1,names=['x','y','z','SM','Metal','u_minus_r'])


data_Groups = pd.read_csv('Data_Groups_and_cluster.csv',header=1,names=['x','y','z','SM','Metal','u_minus_r'])

data_Field = pd.read_csv('Field_galaxies.csv',header=1,names=['x','y','z','SM','Metal','u_minus_r'])

data_Filament = pd.read_csv('Filament_output.csv',header=1,names=['x','y','z','d_per','d_long','length_fil','SM','Metal','u_minus_r'])
...
y1,binEdges1=np.histogram(data_Filament['d_per'],weights=np.zeros_like(data_Filament['d_per']) + 1. / data_Filament['d_per'].size)
bincenters1 = 0.5*(binEdges1[1:]+binEdges1[:-1])
...
ax2.set_xlabel(r'$d_{per}(Mpc)$')
plt.legend(loc=1)
fig2.savefig('histogram_dper.png')
